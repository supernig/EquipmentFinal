...
namespace WindowsFormsApp2
{
    public partial class EquipViewUI : Form
    {
        ...
        public EquipViewUI()
        {
            ...
            setTextform("SELECT items.id,items.name,category.description,items.description,COUNT(itemcontent.id) AS test FROM items left join category on items.categoryID = category.id left join itemcontent on itemcontent.itemID=items.id and items.id ="+Equipment.sendtext+" where itemcontent.tagID < 2 ");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.Yes)
            {
                ...
                refresh("SELECT items.id,itemcontent.modelNumber,itemcontent.id from items left join itemcontent on items.id = itemcontent.itemID where items.id =" + Equipment.sendtext + " and itemcontent.tagID < 2");
                setTextform("SELECT items.id,items.name,category.description,items.description,COUNT(itemcontent.id) AS test FROM items left join category on items.categoryID = category.id left join itemcontent on itemcontent.itemID=items.id and items.id =" + Equipment.sendtext);
        ...
        private void rep_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.Yes)
            {
                string insertQuery = "UPDATE itemcontent SET tagID=1 where itemID =" + Equipment.sendtext + " and id=" + dataGridView3.Rows[dataGridView3.SelectedRows[0].Index].Cells[2].Value.ToString();
